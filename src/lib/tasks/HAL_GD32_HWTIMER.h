...
#define TIMER_RATE_16MHZ_TICKS  4L    // 16L/TIMER_RATE_MHZ, 4x slower than the default 16MHz "sub micros"
  ...
  volatile uint16_t _period0 = 4000, _nextPeriod1 = 4000, _nextPeriod2 = 4000, _nextPeriod3 = 4000, _nextPeriod4 = 4000;
...
bool HAL_HWTIMER_INIT(uint8_t priority, HardwareTimer * hwtimer, void (*wrapper)()) {
    SERIAL_DEBUG.println("******************PASSOUT*****************");
    //hwtimer->pause();
    //hwtimer->setCaptureMode(NC, TIMER_CHANNEL, RISING_EDGE);
    //hwtimer->setCaptureCompare(TIMER_CHANNEL, 1); // Interrupt 1 count after each update   
    //hwtimer->captureCompareCallback();
    //hwtimer->PWM_irqHandle(1, TIMER_CHANNEL)    
    hwtimer->attachInterrupt(wrapper, TIMER_CHANNEL);
    hwtimer->captureCallback(TIMER_CHANNEL);
    ...
    hwtimer->setPeriodTime(_period0, FORMAT_MS); // startup one millisecond
